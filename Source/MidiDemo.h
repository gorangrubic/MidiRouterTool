...
private:
	Slider ti_note_durationSlider;
	Label ti_note_durationLabel;
	Slider ti_note_cycleSlider;
	Label ti_note_cycleLabel;
...
public:
	...
	const unsigned int timer_tick = 250;
          ...
          midiOutputSelector (new MidiDeviceListBox ("Midi Output Selector", *this, false))
    {
		...
		ti_note_durationSlider.setRange(1, 10);		
		ti_note_durationSlider.setNumDecimalPlacesToDisplay(0); // [1]
		ti_note_durationSlider.setTextValueSuffix(" ticks");    // [2]
		ti_note_durationSlider.addListener(this);				// [3]
		addAndMakeVisible(ti_note_durationSlider);

		
		ti_note_durationLabel.setText("Note duration", dontSendNotification);
		ti_note_durationLabel.attachToComponent(&ti_note_durationSlider, true); // [4]
		addAndMakeVisible(ti_note_durationLabel);

		
		ti_note_cycleSlider.setRange(1, 20);					// [1]
		ti_note_cycleSlider.setNumDecimalPlacesToDisplay(0);
		ti_note_cycleSlider.setTextValueSuffix(" ticks");		// [2]
		ti_note_cycleSlider.addListener(this);				// [3]
		
		addAndMakeVisible(ti_note_cycleSlider);

		
		ti_note_cycleLabel.setText("Cycle duration", dontSendNotification);
		ti_note_cycleLabel.attachToComponent(&ti_note_cycleSlider, true);
		addAndMakeVisible(ti_note_cycleLabel);
	...
	bool ti_note_play = true;

	unsigned int ti_note_on = 0;
	unsigned int ti_note_off = 2;

	unsigned int ti_note_cycle = 10;
	unsigned int ti_note_index = 0;

	int ti_note_number = 64;
	int ti_note_channel = 1;
    ...
    void timerCallback() override
    {
		...
		if (ti_note_play) {
			...
			if (ti_note_index == ti_note_on) {
				MidiMessage m(MidiMessage::noteOn(ti_note_channel, ti_note_number, (juce::uint8) 100));
				m.setTimeStamp(Time::getMillisecondCounterHiRes() * 0.001);
				sendToOutputs(m);
				//handleNoteOn(MidiKeyboardState*)
			}

			if (ti_note_index == ti_note_off) {
				MidiMessage m(MidiMessage::noteOff(ti_note_channel, ti_note_number, (juce::uint8) 100));
				m.setTimeStamp(Time::getMillisecondCounterHiRes() * 0.001);
				sendToOutputs(m);
				//handleNoteOn(MidiKeyboardState*)
			}

			if (ti_note_index >= ti_note_cycle) ti_note_index = 0; else ti_note_index++;
    ...
    void handleNoteOn (MidiKeyboardState*, int midiChannel, int midiNoteNumber, float velocity) override
    {
        ...
        m.setTimeStamp (Time::getMillisecondCounterHiRes() * 0.001);
        sendToOutputs (m);
		ti_note_channel = midiChannel;
		ti_note_number = midiNoteNumber;
	...
	{
		if (slider == &ti_note_durationSlider) {
			ti_note_off = ti_note_durationSlider.getValue();
		}
			//ti_note_durationSlider.setValue(1.0 / frequencySlider.getValue(), dontSendNotification);
		else if (slider == &ti_note_cycleSlider) {
			ti_note_cycle = ti_note_cycleSlider.getValue();
		}
			// ti_note_cycleSlider.setValue(1.0 / durationSlider.getValue(), dontSendNotification);
	}
    ...
    void resized() override
    {
        ...
        midiMonitor.setBounds (margin, y,
                               getWidth() - (2 * margin), getHeight() - y - margin - 40);

		
		auto sliderLeft = 120;
		
		ti_note_durationSlider.setBounds(sliderLeft + margin, getHeight() - 40, getWidth() - sliderLeft - margin, 20);
		ti_note_cycleSlider.setBounds(sliderLeft+margin, getHeight() - 20, getWidth() - sliderLeft - margin, 20);
